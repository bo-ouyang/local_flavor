...
const goLoginPage = () => {
  goLogin('/pages/community/publish')
}

const chooseImage = () => {
  if (!ensureAuthed({ toast: '请先登录后上传图片', redirect: '/pages/community/publish' })) return
  uni.chooseImage({
...
const uploadImages = async (paths: string[]) => {
  ...
  for (const path of paths) {
    try {
      ...
      images.value.push(resolveUploadUrl(API_BASE, parseUploadResponse(uploadRes)))
    } catch (error) {
...
const submitPost = async () => {
  if (!ensureAuthed({ toast: '请先登录后发布交流', redirect: '/pages/community/publish' })) return

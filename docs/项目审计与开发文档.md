...
GitHub Actions 质量门禁已在当前 BAS-02 分支实现，位于 `.github/workflows/ci.yml`；尚待 PR 中的真实 Actions 与故障注入验证，不能标记为完成。它对指向 `main` 的 PR 与 `main` push 自动执行：
...
- TruffleHog 只检查本次 PR 或 push 新引入的秘密；扫描失败会阻塞合并。历史密钥的轮换与 Git 历史清理仍属于 BAS-01 外部维护任务。
- `pip-audit` 与 `npm audit` 生成可下载 JSON 报告。Python 3.12 当前解析基线为 0 个已知漏洞，因此 Python 审计立即阻塞；旧版 uni-app 基线为 11 low / 17 moderate / 42 high / 0 critical。npm 命令允许以漏洞退出码完成报告生成，但随后由仓库内基线守卫阻塞任一等级恶化、任何 critical、报告缺失、无效或结构异常。
- 后端或前端门禁失败时保存对应命令日志 14 天；不上传环境文件、数据库、用户上传文件或前端构建产物。依赖审计报告保留 30 天。

仓库管理员应将 `Backend / Python 3.12 / PostgreSQL / Redis`、`Frontend / Node 22 / WeChat mini program`、`Security / New secrets only` 和 `Security / Python dependency gate and npm baseline` 配置为 `main` 分支保护的必需检查。依赖审计作业中的 Python 审计与 npm 基线守卫都会阻塞回归。
